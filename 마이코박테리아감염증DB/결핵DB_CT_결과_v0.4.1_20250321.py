...
# --------------------------------------------------------------------------------
NEGATIVE_PHRASES = ["no", "보이지 않음", "사라졌음", "소실", "없음", "호전"]
VARIABLE = ["변화", "차이", "change", "changes", "거의"]

def split_by_keywords(sentence, keyword_list):
    """
    주어진 문장에서 keyword들이 등장하는 위치 기준으로 문장을 나눈다.
    예: "A는 있지만 B는 없다" → ["A는 있지만", "B는 없다"]
    """
    ...
    for i in range(len(positions)):
        ...
        split_sentences.append(sentence[start:end].strip())
...
def check_keyword(ct_report, keyword, negative_phrases=NEGATIVE_PHRASES):
    ...
    full_exclusion_list = ["cavity", "cavitary mass", "cavities", "emphysema", 
                      "bronchiectasis", "centrilobular nodules", "centrilobular nodules", 
                      "centrilobular nodules", "consolidation", "pleural effusion", "aggravation"]
    ...
    # 제외할 keyword
    exclusion_keywords = [word for word in full_exclusion_list if word not in keyword_list]
    ...
    for sentence in sentences:
        if any(kw.lower() in sentence.lower() for kw in keyword_list):
            if any(ex_kw.lower() in sentence.lower() for ex_kw in exclusion_keywords):
                # keyword 기준으로 문장 분리
                sub_sentences = split_by_keywords(sentence, exclusion_keywords)
                for sub in sub_sentences:
                    if any(kw.lower() in sub.lower() for kw in keyword_list):
                        matched_sentences.append(sub.strip())
            else:
...
condition_A31.to_csv("C:\\Users\\내컴퓨터\\Desktop\\결핵_DB\\DB구축코드\\결핵_DB\\CT_부정어구_v0.1.4_20250321.csv", index=False)

# 부정문과 긍정문이 둘 다 있을 때, 부정문 다음에 긍정문이 나오면 CT소견을 반환하기
