...
def check_keyword(ct_report, keyword, negative_phrases=NEGATIVE_PHRASES):
    ...
    # 현재 문장이 키워드를 포함하는 경우
        if any(kw.lower() in sentence.lower() for kw in keyword_list):
            matched_sentences.append(sentence) # 키워드 포함 문장 자체 저장

            # 다음 문장이 존재하는 경우
            if i + 1 < len(sentences):
                next_sentence = sentences[i + 1] # 다음 문장

            # 다음 문장이 제외할 단어들을 포함하지 않는다면 추가
                if not any(ex_kw.lower() in next_sentence.lower() for ex_kw in exclusion_keywords):
                    matched_sentences.append(sentence + " " + next_sentence)
            
            # bronchiectasis. no interval change in nodules --> Y로 반환해야 하지만 필터링 됨
...
condition_A31.to_csv("C:\\Users\\내컴퓨터\\Desktop\\결핵_DB\\DB구축코드\\결핵_DB\\CT_부정어구_v0.3.1.csv", index=False)
