...
condition_occurrence = pd.read_csv("C:\\Users\\JBCP_01\\Desktop\\CDM_sample\\V2_20241211\\condition_occurrence.csv")
...
conditions = [
    ...
    # {"상병코드": ["R50"], "초진일": "발열"},
    # {"상병코드": ["R042"], "초진일": "객혈"},
    # {"상병코드": ["R06"], "초진일": "호흡곤란"},                                  # 타병원에서 당뇨를 진단받은 경력은 전북대병원 EMR에 반영 안됨, 상병이력 외에 생각해볼 부분은??
    {"상병코드": ["E10", "E11", "E12", "E13", "E14", "O24"], "초진일": "당뇨"},     # 임신성 당뇨는 만성질환으로 간주? 
...
# 반복 작업 수행(# 증상 발현시기 범위 설정 필요)
condition_comorbid = pd.merge(condition_comorbid, condition_A31, on= "person_id", how= 'left')

for cond in symptoms:
    condition_comorbid = condition_comorbid[condition_comorbid["condition_start_date"] <= condition_comorbid["NTM진단일"]]

    temp_df = condition_comorbid[
        condition_comorbid["condition_source_value"].str.startswith(tuple(cond["상병코드"]), na=False)
        ]

    temp_df = temp_df.sort_values(by=["person_id", "condition_start_date"], ascending=[True, False])
...
condition_A31.to_csv("C:\\Users\\JBCP_01\\Desktop\\DB_sample\\기저질환_1.csv", index=False)
